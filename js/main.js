...
$(document).ready(function () {
    ...
    $("#place-order-button").click(function () {
        //URL 'https://jsonplaceholder.typicode.com/posts' is used for testing, after pointing
        // to this URL, the Json is post successfully, and we can see the response in the console.
...
function productRemoved(component) {
    ...
    calculatePrice();
    repaintSelectedList();
    if (orderLineItem.count == 0) disableNonApplePayButton(false);
}
            ...
            var listSelectedItem = 
                '<li>' +
                    '<a href="#">' +
                        '<img src="content/assets/productImages/' + orderLineItem.product.image + '"/>' + 
                        '<h2>' + orderLineItem.product.name + '</h2>' +
                        //Added String "Qty" to be more specific
                        '<p>' +"Qty: "+ (orderLineItem.count) + '</p>' +
...
function calculatePrice() {
    ...
    // Added String "Total" to make it more descriptive.
    ...
    //Added the div into 100.0 to display finalAmount with decimal format in the console
    finalAmount = subTotal/ 100.0;
